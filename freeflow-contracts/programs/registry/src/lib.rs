...
solana_program::declare_id!("HkMhMoEv7U8VowyVsCCk9pZDkWwp18ei1BZ3Fif94DCE");
...
fn process_deregister(program_id: &Pubkey, accounts: &[AccountInfo]) -> ProgramResult {
    ...
    // F1-M2: Verify registry_account is owned by this program before deserializing.
    // Without this, an attacker can craft an account whose data decodes as a
    // RegistryEntry with relay_pubkey == relay_wallet.key.to_bytes(), then call
    // Deregister to drain that account's lamports to the relay_wallet.
    if registry_account.owner != program_id {
        msg!("Deregister: account not owned by registry program");
